...
class PCGSolver: public Solver
{
  public:
	   ...
	   *
	   */
    PCGSolver(StiffnessMatrix &M, Solver &precon, double tol=1e-10, int maxIter=500, bool verbose=true);
    virtual ~PCGSolver();
    ...
    std::tuple<int, double, double> Solve_res(Eigen::VectorXd &b, Eigen::VectorXd &x);

    virtual const std::string  Label () const;
    virtual void SetX0(Eigen::VectorXd x);
