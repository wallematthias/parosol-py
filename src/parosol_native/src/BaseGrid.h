...
public:
  ...
  //!Image dimension
  union{
    struct { t_coord gdim_x, gdim_y, gdim_z; };
    t_coord gdim[3];
  };

  //!Local image dimension
  union{
    struct { t_coord ldim_x, ldim_y, ldim_z; };
    t_coord ldim[3];
  };

  //!Cornercoordinate
  union{
    struct { t_coord corner_x, corner_y, corner_z; };
    t_coord corner[3];
  };

  //!Resolution in all directions. It is usually the same value for all dimension
  union{
    struct{ double res_x, res_y, res_z; };
    double res[3];
  };
