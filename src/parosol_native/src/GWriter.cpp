...
{
  fflush(file);
  //fsync(fileno(file)); REMOVED BY DBETTS. This command forces the OS to write to disk and not cache. This has been removed for two reasons, 1) windows has a different API and 2) The OS should be able to manage it's own memery and write cycles.
}
