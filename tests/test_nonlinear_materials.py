...
from parosol_py.nonlinear import hip_nonlinear, spine_nonlinear


def test_spine_nonlinear_formula_uses_end_to_end_qct_law():
    rho_qct = np.array([[[0.0, 1.0, 2.0]]], dtype=np.float64)

    mapped = spine_nonlinear(rho_qct)

    rho_gcc = rho_qct / 1000.0
    expected_E = 3814.4 * np.power(rho_gcc, 1.05)
    expected_plateau = 57.4464 * np.power(rho_gcc, 1.39)
    ...
    assert mapped.metadata["preset"] == "spine_nonlinear"
    assert mapped.metadata["density_basis"] == "rho_qct_mgcc"
    assert mapped.metadata["side_multiplier"] == pytest.approx(1.28)


def test_hip_nonlinear_femoral_neck_formula_uses_end_to_end_law():
    rho_app = np.array([[[0.0, 1.0, 2.0]]], dtype=np.float64)

    mapped = hip_nonlinear(rho_app, site="femoral_neck")
    ...
    assert mapped.metadata["preset"] == "hip_nonlinear"
    assert mapped.metadata["site"] == "femoral_neck"
    assert mapped.metadata["density_basis"] == "rho_app"


def test_hip_nonlinear_greater_trochanter_formula_uses_end_to_end_law():
    rho_app = np.array([[[0.0, 1.0, 2.0]]], dtype=np.float64)

    mapped = hip_nonlinear(rho_app, site="greater_trochanter")
...
def test_nonlinear_presets_zero_inactive_voxels_from_mask():
    rho_qct = np.ones((1, 1, 3), dtype=np.float64)
    active_mask = np.array([[[True, False, True]]])

    mapped = spine_nonlinear(rho_qct, active_mask=active_mask)
...
def test_hip_nonlinear_rejects_unknown_site():
    rho_app = np.ones((1, 1, 1), dtype=np.float64)

    with pytest.raises(ValueError, match="site must be"):
        hip_nonlinear(rho_app, site="shaft")


def test_nonlinear_presets_reject_non_3d_input():
    with pytest.raises(ValueError, match="rho input must be 3D"):
        spine_nonlinear(np.ones((2, 2), dtype=np.float64))


def test_nonlinear_binning_uses_same_binned_rho_for_all_outputs():
    rho_app = np.array([[[0.0, 1.0, 3.0]]], dtype=np.float64)

    mapped = hip_nonlinear(
        rho_app,
        site="femoral_neck",
        bin_material=True,
        number_bins=1,
